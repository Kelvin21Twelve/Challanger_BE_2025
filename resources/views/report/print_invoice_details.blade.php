...
<head>
    ...
    <title>Invoice @if (!empty($inv_no) && !empty($inv_no))
        @endif
    </title>
...
<body>
    <div class="container">
        <div style="margin-left:20px;">
                ...
                <table class="table_header2" width="100%" id="myTable" cellpadding="20" style="padding-bottom: 30px!important;border: none !important;">
                    <tbody>
                        <tr>
                            ...
                            <img src="{{ $main_array['image']}}" style="max-width: 20%; height:auto; margin-top:10px" style="padding-top: 15px;" ></div></td>
            ...
            @if (!empty($inv_no) && !empty($inv_no))
                <div style="width:100%;margin-bottom: 8px;font-size: 16px;color: black;font-weight: 600;text-align:right;">
                    <span>Invoice No: {{ $inv_no }}</span><span style="float:right;">To Date: {{ $to_date }}</span>
                </div>
            @endif
            ...
            <table class="table_cust" style="width: 100%;border-collapse: collapse;">
                ...
                @php
                    ...
                    $final_amt = 0;
                @endphp
                @foreach ($main_array['data'] as $value)
                    <tr>
                        <td>{{ $i = $i + 1 }}</td>
                        <td>{{ $value->id }}</td>
                        <td>{{ $value->inv_no }}</td>
